...
struct handlers {
  ...
  void finish_pipe_action(auto) {
    ...
    current_pipe.push_back({{[acts = std::move(current_action)] (const value& in) {
  ...
  void finish_seq_action(auto) {
    ...
    current_seq.push_back({{[acts = std::move(current_pipe)] (const value& in) {
  ...
  void finish_seq(auto) {
    ...
    current_seq.clear();
    current_seq.push_back({{[acts = std::move(t)] (const value& in) -> generator<value> {
  ...
  void create_array(auto) {
    ...
    auto t = std::move(current_seq);
    current_seq.clear();
    current_action.push_back({{[acts = std::move(t)] (const value& in) -> generator<value> {
