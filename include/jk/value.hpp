...
using variant = config::
    variant<null_t, int64_t, double, bool, string_type, list_type, map_type>;
...
struct value
{
  ...
  value() noexcept(std::is_nothrow_default_constructible_v<variant>) = default;
  ...
  value(const list_type& v) : v{v} { }
  value(const map_type& v) : v{v} { }
  ...
  value(const string_type& v) : v{v} { }
  value(string_type&& v) noexcept : v{std::move(v)} { }
  value(const string_type::value_type* v) : v{string_type{v}} { }
  value(std::string_view text) : v{string_type{text.data(), text.size()}} { }
  template <typename Traits, typename Allocator>
    requires (!std::is_same_v<std::basic_string<char, Traits, Allocator>, string_type>)
  value(const std::basic_string<char, Traits, Allocator>& text)
      : v{string_type{text.data(), text.size()}} { }
  value(void* v) = delete;
  ...
  value& operator=(const list_type& in) { this->v = list_type{in}; return *this; }
  value& operator=(const map_type& in) { this->v = map_type{in}; return *this; }
  ...
  value& operator=(const string_type& in) { this->v = string_type{in}; return *this; }
  ...
  value& operator=(std::string_view in) { this->v = string_type{in.data(), in.size()}; return *this; }
  template <typename Traits, typename Allocator>
    requires (!std::is_same_v<std::basic_string<char, Traits, Allocator>, string_type>)
  value& operator=(const std::basic_string<char, Traits, Allocator>& in)
  {
    return *this = std::string_view{in.data(), in.size()};
  }
