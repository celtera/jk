...
{
inline
generator<value> process_sequence(const value& in, const std::vector<action_fun>& acts)
{
  DEBUG(" ---- * input: {}\n", to_string(in));
...
inline
generator<value> access_array_range(std::pair<int, int> indices, const value& in)
{
  DEBUG(" ---- * input: {}\n", to_string(in));
  if(auto ptr = get_if<list_type>(&in.v))
  {
...
inline
generator<value> collapse_pipe(const value& in, const std::vector<action_fun>& acts)
{
  DEBUG(" ---- * input: {}\n", to_string(in));
